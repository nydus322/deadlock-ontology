...
(() => {
  ...
  const CLASS_SIZE = {
    Hero: 96, Ability: 68, PropertyCategory: 36, Stat: 34, Slot: 26,
    ScaleFunction: 30, ModifierValue: 26, AbilityProperty: 32,
    AbilityUpgrade: 24, Stage: 22, BlankNode: 16, Resource: 20,
  };
  ...
  function makeCy(elements) {
    // Per-hero accent drives the Map palette: hero node + primary connections
    // all adopt the same colour, making the map feel authored rather than generic.
    const accent = HERO_ACCENT[state.currentHero] || DEFAULT_ACCENT;
    const styleSheet = [
      {
        selector: "node",
        style: {
          "background-color": "#23211c",  // warm charcoal
          "label": "data(label)",
          "color": "#f0e6d8",
          "font-size": 12,
      ...
      // Hero sits at the centre and wears the hero accent.
      { selector: "node.Hero", style: {
      ...
      { selector: "node.Ability", style: {
          "shape": "round-rectangle",
          "font-size": 15,
          "border-color": accent,
          "border-width": 2,
      }},
      { selector: "node.AbilityProperty", style: {
          "shape": "ellipse",
          "background-color": "#1a1712",
          "border-color": "#60503f",
          "font-size": 11,
      }},
      { selector: "node.AbilityUpgrade", style: {
          "shape": "round-rectangle",
          "background-color": "#1a1712",
          "border-color": "#60503f",
          "font-size": 10,
      }},
      { selector: "node.Stat", style: {
          "shape": "round-diamond",
          "background-color": "#1a1712",
          "border-color": "#a3907a",
          "font-size": 11,
      ...
      { selector: "node.collapsed", style: { "opacity": 0.08 } },
      {
        ...
        style: {
          "width": 1.5,
          "line-color": "#3b2f23",
          "target-arrow-color": "#3b2f23",
          "target-arrow-shape": "none",
          "curve-style": "straight",
          "opacity": 0.7,
          // No permanent label on edges — it reads as "what does this arrow
          // mean" noise for non-RDF people. Labels appear on hover only.
          "label": "",
          "font-size": 9,
          "color": "#a3907a",
          "text-background-color": "#12100e",
      ...
      // Signature edges (hero -> ability) render in the hero accent so the
      // kit reads as a constellation around the hero.
      { selector: "edge.hasAbilityInSlot", style: {
          "line-color": accent,
          "width": 2,
          "opacity": 0.8,
      }},
      { selector: "edge.hasProperty", style: { "line-color": "#60503f" } },
      { selector: "edge.hasUpgrade",  style: { "line-color": "#3b2f23", "line-style": "dashed" } },
      { selector: "edge.scalesStat", style: {
          "line-color": "#d89b4a",
          "target-arrow-shape": "triangle",
          "target-arrow-color": "#d89b4a",
          "opacity": 0.9,
      }},
      { selector: "edge:selected", style: {
          "line-color": accent, "target-arrow-color": accent,
          "width": 3, "text-opacity": 1,
      }},
      { selector: "edge.hidden", style: { "display": "none" } },
      { selector: "edge.collapsed", style: { "opacity": 0.05 } },
      // Only show edge label when hovered (handled below via class toggle).
      { selector: "edge.highlighted", style: { "label": "data(label)", "text-opacity": 1 } },
    ];
  ...
  // The raw RDF topology is Hero -> hasAbilityInSlot bnode -> {slot, ability},
  // which renders in Cytoscape as a bnode between each hero and slot. The
  // user's mental model is Hero -> Slot -> Ability: synthesize that by
  // removing the bnode intermediary and patching hero -> slot and slot ->
  // ability edges in its place. All other RDF structure (ability -> properties,
  // property -> scalesStat, etc.) is preserved.
  function synthesizeKitTopology(graph) {
    const nodeById = new Map(graph.nodes.map((n) => [n.data.id, n.data]));
    const outgoing = new Map();
    graph.edges.forEach((e) => {
      const s = e.data.source, t = e.data.target, p = e.data.label;
    ...
    const heroNode = graph.nodes.find((n) => (n.data.classes || "").split(/\s+/).includes("Hero"));
    if (!heroNode) return { nodes: graph.nodes, edges: graph.edges };
    const heroId = heroNode.data.id;

    const bnodeIds = new Set();
    const syntheticEdges = [];

    (outgoing.get(heroId) || []).forEach(([bn, pred]) => {
      if (pred !== "hasAbilityInSlot") return;
      ...
      bnodeIds.add(bn);
      const slotId = slotEdge[0];
      const abId = abEdge[0];
      const slotLabel = nodeById.get(slotId)?.label || "slot";
      const abLabel = nodeById.get(abId)?.label || "ability";
      // hero -> slot (labelled with the slot name for readability)
      syntheticEdges.push({
        data: {
          id: `syn-hs-${slotId}`,
          source: heroId,
      ...
      // slot -> ability (labelled with the ability name)
      syntheticEdges.push({
        data: {
    ...
    // Drop the bnodes and any edge touching them — they're replaced by the
    // synthetic pair above.
    const nodes = graph.nodes.filter((n) => !bnodeIds.has(n.data.id));
    const edges = graph.edges
      .filter((e) => !bnodeIds.has(e.data.source) && !bnodeIds.has(e.data.target))
  ...
  function switchHero(codename, { fromHash = false } = {}) {
    ...
    // Synthesize Hero -> Slot -> Ability topology by collapsing the RDF
    // hasAbilityInSlot blank nodes. This is what Cytoscape renders.
    const topo = synthesizeKitTopology(state.graph);
    state.cy = makeCy([...topo.nodes, ...topo.edges]);
    registerCyEvents();
    ...
    // Hierarchical tree rooted at the hero: Hero -> Ability bindings ->
    // Ability -> properties/upgrades. Reads as a family tree, not a cloud.
    const rootNode = state.cy.getElementById(rootId());
    state.cy.layout({
      name: "breadthfirst",
      roots: rootNode,
      directed: true,
  ...
  async function init() {
    try {
      ...
      document.getElementById("btn-reset").addEventListener("click", () => {
        const rn = state.cy.getElementById(rootId());
        state.cy.layout({
          name: "breadthfirst",
          roots: rn,
          directed: true,
      ...
      document.getElementById("btn-kit-mode").addEventListener("click", () => {
        ...
        const rn = state.cy.getElementById(rootId());
        state.cy.layout({
          name: "breadthfirst", roots: rn, directed: true,
          padding: 48, spacingFactor: 1.4,
